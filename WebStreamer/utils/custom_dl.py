...
class ByteStreamer:
    ...
    async def get_file_properties(self, channel_id: int, message_id: int) -> FileId:
        """
        ...
        if message_id not in self.cached_file_ids:
            await self.generate_file_properties(channel_id, message_id)
            logger.debug(f"Cached file properties for message with ID {message_id}")
        return self.cached_file_ids[message_id]
    
    async def generate_file_properties(self, channel_id, message_id: int) -> FileId:
        """
        Generates the properties of a media file on a specific message.
        returns ths properties in a FIleId class.
        """
        file_id = await get_file_ids(self.client, int(f'-100{channel_id}'), message_id)
